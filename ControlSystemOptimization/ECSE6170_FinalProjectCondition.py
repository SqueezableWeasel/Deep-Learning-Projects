...
#Get the A-matrix condition and PMU.y response of the control system
def get_ybar(gain,a,b,c,d,e,ff,g,h,i,j,k,l):
    ...
    try:
        ...
        for i in mat["data_2"][10]:
            q.append(i)
        #Get state space matrices from mat file, compute condition of A matrix
        ...
        if dymola is not None:
            dymola.close()
            dymola = None
    #Return array of form (Condition(A),X_1,X_2...X_502)
    q = np.insert(q, 0, c, axis=0)
    return q
...
for i in range(0,n):
    ...
    X[i] = ybar
    print(Y[i],X[i])
np.save("X_5std6000_condition.npy",X)
np.save("Y_5std6000_condition.npy",Y)
'''
#Load the dataset
X = np.load("X_5std6000_condition.npy")
Y = np.load("Y_5std6000_condition.npy")
...
plt.plot(qq[1:],label="Original Model")
plt.plot(qhat_out[1:],label="Predicted Model")
plt.legend(loc="upper right")
t = "Control System Response \n MAPE: "+str(MAPE(qq[1:],qhat_out[1:]))
plt.title(t)
plt.savefig('2model_1.png')
plt.clf()

#For 50 iterations, find a good optimized model
for i in range(0,50):
    ...
    try:
        ...
        #Get model prediction from z
        q = model.predict(asarray([z]))
        #Get system response given predicted parameters
        qbar = get_ybar(q[0][0],q[0][1],q[0][2],q[0][3],q[0][4],q[0][5],q[0][6],q[0][7],q[0][8],q[0][9],q[0][10],q[0][11],q[0][12])
        axs[0].plot(qbar[1:],label="Model Prediction")
        axs[0].legend(loc="upper right")
        axs[1].set_ylim([-.1, .5])
        axs[1].plot(qq[1:],label="Original Model")
        axs[1].legend(loc="upper right")
        plt.savefig('2model_22.png')
        plt.clf()
        #Print model prediction statistics
        if 100-max(qbar[250:])*100 < 100:
            print("Original model condition:",157)
            print("Optimized model condition:",qbar[0])
            #if the condition is less than 1000 and the optimized model performance is better than the original:
            if qbar[0] < 1000 and 100-max(qbar[250:])*100 > 100-max(qq[250:])*100:
                #Store the model in the dictionary based on the % FO suppression / the condition
                p[(100-max(qbar[250:])*100)/qbar[0]] = (100-max(qbar[250:])*100,qbar[0],q[0])
...
for key in sorted(p):
    print("#############")
    print("FO suppression/condition:",key)
    print("FO suppression:",p[key][0])
    print("Condition:",p[key][1])
    print("Parameters:",p[key][2])
